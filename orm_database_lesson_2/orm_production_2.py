...
import orm_models_2, orm_schemas_2
...
@router.get("/user reg data", status_code=status.HTTP_200_OK, response_model=List[orm_schemas_2.UserRegData])
def get_all_user_reg_data_info(db: Session = Depends(get_database)):
    user_reg_data = db.query(orm_models_2.User_Reg_Data).order_by(orm_models_2.User_Reg_Data.id).all()
    return user_reg_data


@router.get("/personal data", status_code=status.HTTP_200_OK, response_model=List[orm_schemas_2.UserPersonalData])
def get_all_personal_data_info(db: Session = Depends(get_database)):
    personal_data =db.query(orm_models_2.Personal_Data).order_by(orm_models_2.Personal_Data.id).all()
    return personal_data


@router.get("/user posts", status_code=status.HTTP_200_OK, response_model=List[orm_schemas_2.UserPosts])
def get_all_user_posts_info(db: Session = Depends(get_database)):
    user_posts = db.query(orm_models_2.User_Posts).order_by(orm_models_2.User_Posts.id).all()
    return user_posts
...
def delete_post(username_or_email: str|EmailStr, post_title: str, db: Session = Depends(get_database)):
    ...
    all_posts = db.query(orm_models_2.User_Posts).filter(orm_models_2.User_Posts.postTitle == post_title).all()
