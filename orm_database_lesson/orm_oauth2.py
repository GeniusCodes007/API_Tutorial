...
import orm_schemas, orm_database, orm_models
...
def verify_token(token: str, credentials_exception):
    try:
        ...
        token_data = orm_schemas.TokenResponse(access_token=token, token_type="bearer", id_=str(my_id))
    except JWTError:
        raise credentials_exception
    return token_data

def get_current_user(token: str = Depends(oauth2_scheme), db: Session = Depends(orm_database.get_database)):
    ...
    user=db.query(orm_models.User_Reg_Data).filter(orm_models.User_Reg_Data.id == token.id_).first()
